    ...
    <main class='d-flex justify-content-center align-items-center'>
        <form action='./../include/validator.php' method='POST' enctype='multipart/form-data'>
            <h4>Adding a New orders</h4>
				...
				<section class='form-floating'>
                        <input type='text' class='form-control' name='total' id'total' placeholder='i' required/>
                        <label for='total'>total</label>
                        </section>
				<section class='form-floating'>
                        <input type='text' class='form-control' name='payment' id'payment' placeholder='i' required/>
                        <label for='payment'>payment</label>
                        </section>
				<section class='form-floating'>
                        <input type='text' class='form-control' name='paymentDate' id'paymentDate' placeholder='i' required/>
                        <label for='paymentDate'>paymentDate</label>
                        </section>
<section class='form-floating'>
                <button class='btn btn-success' name='postorders'>Add</button>
                <section class='btn btn-primary' onclick='window.location = `./../index.php`'>Home</section>
            </section>
            <?php 
            if(isset($_COOKIE['ordersUpload']) && $_COOKIE['ordersUpload'] == 'success'){
                echo '
